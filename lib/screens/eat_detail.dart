...
class EatDetail extends StatelessWidget {
  const EatDetail({
    super.key,
    required this.item,
  ...
  Widget build(BuildContext context) {
    ...
    final tripAdvisorBtn = _getSocialNetworkButton(
      item.tripAdvisorUrl,
      const Icon(
    ...
    final igButton = (item.instagramUrl != null)
        ? _getSocialNetworkButton(
            item.instagramUrl!,
            const Icon(
        ...
        : const SizedBox();
    final fbButton = (item.facebookUrl != null)
        ? _getSocialNetworkButton(
            item.facebookUrl!,
            const Icon(
        ...
        : const SizedBox();
    final twitterBtn = (item.twitterUrl != null)
        ? _getSocialNetworkButton(
            item.twitterUrl!,
            const Icon(
    ...
    if (accessible) {
      if (item.shortDescriptionEn != null && item.shortDescription != null) {
        description = (language.english)
            ? item.shortDescriptionEn!
            : item.shortDescription!;
      }
    } else {
      if (item.longDescriptionEn != null && item.longDescription != null) {
        description = (language.english)
    ...
    if (item.schedule != null && item.schedule!.isNotEmpty) {
      ...
      if (item.holidays != null && item.holidaysEn != null) {
        vacationLabel = (language.english)
            ? item.holidaysEn ?? AppConstants.noVacationDataLabelEn
            : AppConstants.noVacationDataLabel;
      }
    } else {
      vacationText = Align(
    ...
    if (item.canReserve != null && item.canReserve!) {
      reservationForm = ReservationForm(restaurantId: item.placeId);
    }
    ...
    return Scaffold(
      appBar: AppBar(
        leading: Semantics(
          label: 'Botón para volver a la lista de restaurantes',
          hint: 'Toca para regresar a la lista de restaurantes',
          button: true,
          child: IconButton(
            icon: const Icon(
        ...
        title: Semantics(
          label: 'Título del restaurante: ${item.placeName}',
          child: DynamicTitle(
        ...
        child: Padding(
          ...
          child: Center(
            child: Column(
              children: [
                Semantics(
                  label: 'Imagen principal del restaurante ${item.placeName}',
                  child: ClipRRect(
                ...
                spacer,
                Semantics(
                  label: 'Descripción del restaurante: $description',
                  child: Padding(
                    padding: const EdgeInsets.only(
                      top: AppConstants.spacing,
                      left: AppConstants.spacing,
                ...
                spacer,
                Semantics(
                  label: 'Dirección del restaurante: ${item.address}',
                  child: Row(
                    children: [
                      const Icon(
                        Icons.location_on,
                ...
                spacer,
                Semantics(
                  label: 'Enlaces a redes sociales y sitio web del restaurante',
                  child: Row(
                    children: [
                      tripAdvisorBtn,
                      webButton,
                ...
                spacer,
                Semantics(
                  label: 'Ofertas y promociones del restaurante',
                  child: Offers(item: item),
                ),
                spacer,
                Semantics(
                  label: 'Galería de imágenes del restaurante',
                  child: gallery,
                ),
                spacer,
                Semantics(
                ...
                spacer,
                Semantics(
                  label: 'Panel de calificaciones y reseñas',
                  child: RatingPanel(restaurantId: item.placeId),
                ),
              ],
            ),
          ),
